...
const LoginPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Only call loginUser; useEffect will handle redirect
    loginUser(data);
    setLoading(false);
  ...
  // Show a loader while redirecting if user is already logged in
  if (userInfo) {
    ...
    <div className="flex items-center justify-center min-h-[calc(100vh-10rem)] py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8 bg-white p-10 rounded-lg shadow-md">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="h-6"> {/* Spacer to avoid form jump when register link is absent */}
            {!isAdminLoginPath && (
