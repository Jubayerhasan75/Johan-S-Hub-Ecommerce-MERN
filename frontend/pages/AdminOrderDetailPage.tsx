...
import { 
  ...
  XCircle, 
  ArrowLeft,
  AlertTriangle, // Notun icon
  Hash // Notun icon
} from 'lucide-react';
import { Order } from '../types';
import { useAppContext } from '../context/AppContext';
import { API_BASE_URL } from '../constants'; // <-- Live link import
...
const AdminOrderDetailPage: React.FC = () => {
  ...
  const [deleteLoading, setDeleteLoading] = useState(false); // Delete state
  ...
  const handleMarkAsDelivered = async () => {
    ...
    // --- NOTUN CHECK: Manual Payment hole warning ---
    if (order.paymentMethod === 'Manual Payment' && !order.isPaid) {
      if (!window.confirm('This is a Manual Payment order. Have you verified the Transaction ID and received the payment? Clicking OK will mark this order as PAID and DELIVERED.')) {
  ...
  const handleDeleteOrder = async () => {
    ...
    if (window.confirm('Are you sure you want to permanently delete this order? This action cannot be undone.')) {
      ...
      try {
        ...
        navigate('/admin/orders'); // Ferot jan
    ...
    <div className="container mx-auto px-4 py-8 sm:py-12 max-w-5xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* --- NOTUN PAYMENT DETAILS SECTION --- */}
          <div className="bg-white p-6 rounded-lg shadow-md">
            <h2 className="text-xl font-semibold mb-4">Payment Details</h2>
            <div className="space-y-2">
              <p><strong>Payment Method:</strong> {order.paymentMethod}</p>
              
              {/* Jodi Manual Payment hoy, TrxID dekhano */}
